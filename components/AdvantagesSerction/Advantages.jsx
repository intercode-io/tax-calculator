...
export default function Advantages() {
    const getAdvantagesData = useFetchData('/homepage', ['Advantages.AdvantagesList.Image']);
    const advantagesData = getAdvantagesData?.Advantages;
            ...
            <div className={style.advantages__items}>
                {advantagesData?.AdvantagesList.map((advantage) => (
                    <div key={advantage.id} className={style.item}>
                        <img src={`${process.env.NEXT_PUBLIC_STRAPI_API_URL}${advantage?.Image?.data?.attributes?.url}`}
